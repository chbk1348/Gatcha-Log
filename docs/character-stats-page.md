...
> ⚠️ **작업 범위**: 현재 **스탯 페이지 디자인/UI 는 맥(Mac) 환경에서 직접 구성 중**이다.
> 따라서 이 작업에서는 **디자인·UI 코드(Compose 화면, SwiftUI, 레이아웃/스타일)는 건드리지 않는다.**
> 범위는 **데이터 레이어(API 클라이언트·파싱·모델·KR 용어 매핑/포맷 유틸)** 에 한정한다.
...
- **예외(앱 통일 결정)**: HP 스탯은 **두 게임 모두 "HP"로 통일 표기**. 원신 인게임 공식은 "생명력"이나, 앱 일관성을 위해 의도적으로 "HP"를 사용한다. *(추후 "생명력"으로 되돌리지 말 것)*
...
| propId | 의미 | KR 표기 | 포맷 |
|---|---|---|---|
| `2000` | 최대 HP(생명력) | **HP** *(앱 통일 표기 — 인게임은 "생명력")* | 정수 |
| `2001` | 최종 공격력 | **공격력** | 정수 |
| `2002` | 최종 방어력 | **방어력** | 정수 |
...
> 하드코딩하지 않고 mihomo 응답의 KR 라벨을 그대로 노출. 아래는 **검증용 참고 목록**.
> 출처: **StarRailRes `index_min/kr`**(게임 파일에서 추출한 실제 인게임 KR 텍스트) + 나무위키 대조.

### 4-1. 핵심 스탯 (인게임 공식 표기 — 띄어쓰기 포함 그대로)
| 영문 | KR 표기(인게임) |
|---|---|
| HP | **HP** |
| ATK | **공격력** |
...
| CRIT Rate | **치명타 확률** |
| CRIT DMG | **치명타 피해** |
| Effect Hit Rate | **효과 명중** *(주의: "효과 적중" 아님)* |
| Effect RES | **효과 저항** |
| Break Effect | **격파 특수효과** *(붙여쓰기)* |
| Energy Regen Rate | **에너지 회복효율** *(붙여쓰기)* |
| Outgoing Healing | **치유량 보너스** *(주의: "치유량 추가" 아님)* |

### 4-2. 속성 피해 증가 (※ 원신은 "보너스", 스타레일은 "증가" — 게임별 다름!)
- **물리 속성 피해 증가**
- **화염 속성 피해 증가**
- **얼음 속성 피해 증가**
- **번개 속성 피해 증가**
...
### 4-3. 유물 부위
- 유물 4부위: **머리 · 손 · 몸통 · 발** (StarRailRes 타입코드 HEAD/HAND/BODY/FOOT)
- 차원 장식물 2부위: **차원계 구체 · 연결 로프** (타입코드 NECK/OBJECT)
- 슬롯 라벨은 런타임에 mihomo `lang=kr` 응답값을 따르는 것을 원칙으로 함.
...
- ~~공용 캐릭터 스탯 상세 화면 Composable~~ → **범위 외**: UI/디자인은 맥 환경에서 직접 구성(이 작업은 데이터 레이어만 담당)

---

## 9. 용어 검증 출처

- **스타레일**: `Mar-7th/StarRailRes` `index_min/kr/properties.json`(게임 파일 추출 실제 인게임 KR 텍스트)로 검증 → "효과 명중 / 치유량 보너스 / 격파 특수효과 / 에너지 회복효율 / OO 속성 피해 증가" 확정. 나무위키와 교차 확인.
- **원신**: 나무위키 `원신/성유물`·`원신/원소`로 교차 확인 → "불 원소 피해 보너스 / 물리 피해 보너스 / 원소 마스터리 / 공간의 성배·이성의 왕관" 등 기존 표기 정확.
- **게임 간 차이 주의**:
  - HP 스탯: 인게임 공식은 **원신="생명력" / 스타레일="HP"** 로 다르나, **본 앱은 두 게임 모두 "HP"로 통일**(의도적 결정 — 원신 "생명력"을 "HP"로 표기)
  - 원소/속성 피해 증가분: **원신="원소 피해 보너스" vs 스타레일="속성 피해 증가"**
  - 치유 스탯: **원신="치유 보너스" vs 스타레일="치유량 보너스"**
