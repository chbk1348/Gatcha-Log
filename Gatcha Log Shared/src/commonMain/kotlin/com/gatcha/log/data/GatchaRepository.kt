...
class GatchaRepository(
     ...
     * ## 마이그레이션이 붙은 키를 다룰 때
     *
     * **값만 싣지 말고 그 값을 해석하는 기준(버전)도 함께 싣는다.** 스냅샷은 저장소 마이그레이션과
     * 별개 경로라, 변환 코드만 고치면 이쪽을 놓치기 쉽다 — 실제로 강조색에서 한 번 났다.
     *
     * export 가 내보내는 값은 대개 **이미 변환된 값**이다(`loadAccentIndex` 처럼 읽기 함수가
     * 변환까지 한다). 그런데 import 가 버전 플래그를 함께 복원하지 않으면 받는 쪽 버전이 0 으로
     * 남아 **이미 변환된 값에 변환이 또 걸린다.** 증상은 두 가지로 나온다.
     *
     *  1. 클라우드에서 복원할 때마다 값이 한 칸씩 밀린다(강조색: 틸 → 머스터드).
     *  2. 두 기기가 서로의 스냅샷을 계속 '변경'으로 인식해 **끝없이 밀어 올린다**
     *     ([GatchaRepositorySnapshotTest] 의 왕복 멱등성이 이걸 잡는다).
     *
     * 반대로 import 에서 버전을 무조건 현재값으로 박아도 안 된다 — 옛 스냅샷의 **필요한** 변환까지
     * 건너뛰어 값이 틀어진다. 기준을 실제로 실어 나르고, 키가 없으면(옛 스냅샷) 0 으로 읽는다.
     *
     * 즉 새 마이그레이션을 붙일 때 손댈 곳은 세 군데다: 읽기 함수 · [exportSnapshot] · [importSnapshotJson].
     */
