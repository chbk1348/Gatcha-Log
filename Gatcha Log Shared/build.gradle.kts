...
kotlin {
    ...
    android {
        ...
        compileSdk = 36 // :Gatcha Log Android(36)과 정합 — 단일 클래스패스 소비
        minSdk = 31 // :Gatcha Log Android 와 동일 — "최소 지원 OS"의 답이 두 개가 되지 않게 맞춘다
        ...
        // commonTest 를 JVM 호스트에서 실행 (:Gatcha Log Shared:testAndroidHostTest).
    ...
    listOf(
        ...
        iosTarget.binaries.framework {
            baseName = "Shared"
            isStatic = true
            // 프레임워크 bundle ID 명시(없으면 SKIE/링커가 경고). :Gatcha Log Android 와 무관한 공유 모듈 식별자.
    ...
    sourceSets {
        commonMain.dependencies {
            ...
            // 날짜/시간 — :Gatcha Log Android 의 Calendar/SimpleDateFormat 대체
            implementation("org.jetbrains.kotlinx:kotlinx-datetime:0.8.0")
            // JSON — :Gatcha Log Android 의 org.json 대체 (com.gatcha.log.json 호환 레이어가 감쌈)
            implementation("org.jetbrains.kotlinx:kotlinx-serialization-json:1.11.0")
            // HTTP 클라이언트 — :Gatcha Log Android 의 HttpURLConnection 대체 (Net 호환 레이어가 감쌈)
            ...
            // ── 클라우드 (5단계) ──
            // Firebase 인증 + Firestore — :Gatcha Log Android 의 com.google.firebase 를 GitLive(KMP) 로 대체
        ...
        androidMain.dependencies {
            ...
            // com.google.firebase:* 버전을 고정 (:Gatcha Log Android 과 동일 BoM). 없으면 androidCompileClasspath 해석 실패.
            implementation(project.dependencies.platform("com.google.firebase:firebase-bom:34.14.0"))
            // 보안 토큰 저장(EncryptedSharedPreferences) — :Gatcha Log Android 과 동일 버전 유지 (SecureKeyValueStore actual).
