...
/**
 ...
 목록과 상세의 출처가 다르다. 목록은 hakush 미러 인덱스(한국어 이름 포함), 상세는 nanoka
 단건이다 — nanoka 에 목록 API 가 없어서 이렇게 나뉜다.
 */
struct BangbooPage: View {
    var store: SpendingStore
    @Environment(\.glgAccent) private var accent
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                Text("젠레스 존 제로의 방부\(store.bangboo.isEmpty ? "" : " \(store.bangboo.count)종") — 탭하면 스탯과 스킬을 봅니다.")
                    .font(.pretendard(size: 12)).foregroundStyle(GLGColor.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
                    .padding(.bottom, 14)
                if store.bangboo.isEmpty {
                    if store.bangbooLoading {
                        ProgressView().controlSize(.small).tint(accent.primary)
                            .frame(maxWidth: .infinity).padding(.top, 40)
                    } else {
                        Text("목록을 받지 못했어요.")
                    ...
                    GLGCard(cornerRadius: 20, padding: 16) {
                        VStack(alignment: .leading, spacing: 0) {
                            ForEach(Array(store.bangboo.enumerated()), id: \.offset) { i, b in
                                if i > 0 { Divider() }
                                row(b)
                            }
                        }
        ...
        .scrollIndicators(.hidden)
        .background(GLGBackground { Color.clear })
        .navigationTitle("방부 도감")
        .navigationBarTitleDisplayMode(.inline)
        .task { store.loadBangboo() }
    }

    @ViewBuilder
    private func row(_ b: BangbooEntry) -> some View {
        let expanded = open == b.id
        let detail = store.bangbooDetail[b.id]
        VStack(alignment: .leading, spacing: 0) {
            Button {
                open = expanded ? nil : b.id
                if !expanded { store.loadBangbooDetail(b.id) }
            } label: {
                HStack(spacing: 10) {
                    Text(b.rankLabel.isEmpty ? "-" : b.rankLabel)
                        .font(.pretendard(size: 10.5, weight: .bold))
                        .foregroundStyle(b.rank >= 4 ? accent.primary : GLGColor.textSecondary)
                        .padding(.horizontal, 7).padding(.vertical, 2)
                        .background(b.rank >= 4 ? accent.primary.opacity(0.16) : Color(hex: 0xFFF0F1F4),
                                    in: RoundedRectangle(cornerRadius: 7, style: .continuous))
                    Text(b.name).font(.pretendard(size: 13.5, weight: .bold)).foregroundStyle(GLGColor.textPrimary)
                    Spacer(minLength: 8)
                    Text(b.codeName).font(.pretendard(size: 10.5)).foregroundStyle(GLGColor.textSecondary)
                }
            ...
            if expanded {
                if let d = detail {
                    VStack(alignment: .leading, spacing: 0) {
                        if !d.desc.isEmpty {
                            Text(d.desc).font(.pretendard(size: 12)).foregroundStyle(GLGColor.textSecondary)
                                .fixedSize(horizontal: false, vertical: true).padding(.bottom, 9)
                        }
                        HStack(spacing: 8) {
                            stat("HP", Int(d.hp))
                            stat("공격력", Int(d.atk))
                        ...
                        ForEach(Array(d.skills.enumerated()), id: \.offset) { _, sk in
                            VStack(alignment: .leading, spacing: 3) {
                                Text(sk.name).font(.pretendard(size: 12, weight: .bold)).foregroundStyle(accent.primary)
                                Text(sk.desc).font(.pretendard(size: 12)).foregroundStyle(GLGColor.textSecondary)
                                    .fixedSize(horizontal: false, vertical: true)
                            }
                            .padding(.top, 10)
                        }
                    ...
                    ProgressView().controlSize(.mini).tint(accent.primary).padding(.top, 10)
                }
            }
        }
        .padding(.vertical, 13)
    ...
    private func stat(_ label: String, _ value: Int) -> some View {
        ...
        .background(Color(hex: 0xFFF7F8FA), in: RoundedRectangle(cornerRadius: 10, style: .continuous))
    }
}
