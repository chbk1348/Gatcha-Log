    ...
    GlgDialog(
        ...
        Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
            ...
            UplogEntry(
                "v27.20.1",
                listOf(
                    "[보안] 정품 릴리스 키로 전환했어요 — 위·변조된 업데이트 설치를 막아요 (이 버전은 1회 재설치가 필요해요)",
                    "인증 토큰이 평문으로 저장·백업되던 경로를 차단했어요",
                    "내려받은 설치 파일을 SHA-256으로 검증한 뒤 설치해요",
                ),
            )
            UplogEntry(
                "v27.20.0",
                listOf(
                    "마이페이지를 새로 정리했어요 — 계정 정보를 상단 프로필 카드 한 곳으로 (로그인·로그아웃도 여기서)",
            ...
            UplogEntry(
                "v27.1.1",
                listOf(
                    "홈 프로필 영역을 개편했어요 (인사 히어로 카드)",
                    "당겨서 새로고침을 추가했어요 (홈·지출·게임 정보)",
                    "지출 상세 필터와 상단 정리(접이식 섹션)를 더했어요",
                    "지출 카드·태그·메뉴 디자인을 개선했어요",
                    "재화 가격·명칭을 정정했어요 (월정액 5,900원, 창세의 결정 등)",
                    "기기 글꼴 크기 영향을 제거하고 구글 프로필 사진을 표시해요",
                ),
            )
            UplogEntry(
                "v27.1.0",
                listOf(
                    "첫 출시 — 가챠 지출 트래커, 호요버스 게임을 한 곳에서 관리해요",
                    "지출 · 예산 · 연간 리포트 · 구독 관리",
                    "게임 정보 — 배너 · 실시간 노트 · 출석 · 연속 출석",
                    "가챠 확률표 · 통합 계산기 · 천장 카운터",
                    "프로필 쇼케이스(Enka) · 가챠 효율 리포트(UIGF/SRGF)",
                    "구글 계정 클라우드 동기화 · 인앱 자동 업데이트",
                ),
            )
        }
    }
