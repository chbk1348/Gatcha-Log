...
object GachaReport {
    ...
    // ----------------------------------------------------------------- 통계
    /**
     * 스노우플레이크 ID 숫자 정렬 키 — (자릿수 3자리 패딩 + 숫자 문자열)의 사전순 == 숫자순.
     * :app 은 BigInteger 를 쓰지만 KMP common 엔 없어서 문자열 비교 방식으로 대체 (동일한 정렬 결과).
     */
    private fun bigId(s: String): String {
        val t = s.trim().filter { it.isDigit() }.trimStart('0').ifEmpty { "0" }
